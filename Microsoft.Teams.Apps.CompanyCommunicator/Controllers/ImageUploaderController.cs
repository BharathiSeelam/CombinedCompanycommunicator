        ...
        private async Task<(bool, string)> UploadToBlob(string filename, string stream = null)
        {
            ...
            if (CloudStorageAccount.TryParse(this.storageAccount, out storageAccount))
            {
                try
                {
                    ...
                    var bytes = Convert.FromBase64String(stream.Replace("data:image/jpeg;base64,", string.Empty).Replace("data:image/png;base64,", string.Empty).Replace("data:image/bmp;base64,", string.Empty)); // without data:image/jpeg;base64 prefix, just base64 string.
